...
`ai_robot_task_manager` 已实现Action服务端、Goal校验、单任务状态机、取消标记、所有权清理和`mode:=sim|real`独立启动入口。Nav2和到点感知尚未接入；因此当前合法Goal在校验后返回`ERROR_NAVIGATION_REJECTED`，而不是伪造成功。该行为是M6待关闭项第3项之前的显式安全边界。
